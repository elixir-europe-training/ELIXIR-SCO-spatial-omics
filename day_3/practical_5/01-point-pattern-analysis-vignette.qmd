...
# 2. how many cells are annotated as 'OD Mature' in each of 
#    the 3 slices used below (-0.09, 0.01, 0.21)?
```


Next, we want to extract three slices of this `r BiocStyle::Biocpkg('SpatialExperiment')` object and convert the 2D slices into `ppp` objects. The `r BiocStyle::Biocpkg('spatialFDA')` package contains a function for this conversion: `spatialFDA:::.dfToppp`.
...
selectZstacks <- function(zstack, spe) {
  subset_spe <- spe[, spe$sample_id == zstack]
  df <- spatialFDA:::.speToDf(subset_spe)
...
EXERCISE: Try to compare the arrangement of `Ependymal` cells across these three sections. You can choose any function you might find in the documentation of `r BiocStyle::CRANpkg('spatstat.explore')`. For example, look for the **Summary statistics for a point pattern** section of the help page at `?spatstat.explore`.
...
# EXERCISE: 
# 1. Play with the scale of the curves (e.g., although we've used L, which is a variance-stabilized transform of the K function,  ); for example, consider `sqrt()` or `asinh(. / 50)` or similar. Recompute 
...
EXERCISE: try to compare the colocalisation of `OD Mature` and `Ependymal` cells. Again, the documentation of `r BiocStyle::CRANpkg('spatstat.explore')` will help you to find good cross functions and `calcMetricPerFov` is a helper (wrapper) function that is useful to operate on `r BiocStyle::Biocpkg('SpatialExperiment')` objects.  Similar to above, look for the **Summary statistics for a multitype point pattern** section of the help page at `?spatstat.explore`.
