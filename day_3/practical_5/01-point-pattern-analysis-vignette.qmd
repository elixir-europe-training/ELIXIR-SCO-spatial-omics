...
author: "Mark D. Robinson (Martin Emons, Samuel Gunz)"
format:
    html:
      toc: true
      self-contained: true
...
source(file.path(basedir,"code/utils.R"))
...
# 2. how many cells are annotated as Astrocytes in each of 
#    the 3 slices used below (-0.09, 0.01, 0.21)?
```


Next, we want to extract three slices of this `r BiocStyle::Biocpkg('SpatialExperiment')` object and convert the 2D slices into `ppp` objects. Note that in the `code/utils.R` script, we have defined a function called `.ppp()`.
...
#    (hint: see `?grDevices::chull`)
# 2. Set the convex hull as the Window() of the ppp object
# polygon(X[chull(X), ])

# ch <- spatialCoords(spe) %>% chull
# plot(pp_sub)
# polygon(spatialCoords(spe)[ch,])
...
The next concept that defines a point pattern is that **marks** can be associated with the points. Point patterns can even be unmarked (e.g., `unmark(pp_sub)`).

In the context of cell biology, we can distinguish between discrete marks (e.g., cell types) or continuous marks (e.g., gene expression).
...
# EXERCISE:
# 1. Adjust the code blocks above to also visualize 
#    the expression of 'Prlr'
```
...
```{r, fig.height=5, fig.width=10}
# EXERCISE
# 1. Extract the expression of Esr1 across 
# the same three slices as above
```
...
p <- ggplot(dfm, aes(x = r, y = asinh(value/50), 
...
df_fdob <- df %>% as.matrix %>% as.data.frame()
...
p_biplot <- ggplot(scores_df, 
                   aes(scores_df[, 1], scores_df[, 2], 
                       colour = (dat[['sel']]))) +
  geom_point() +
  coord_equal() +
  theme_light() +
...
# EXERCISE: 
# 1. Play with the scale of the curves (e.g., although we've used L, which is a variance-stabilized transform of the K function,  ); for example, consider `sqrt()` or `asinh(. / 50)` or something similar. Recompute 
...
```{r, message = FALSE, warning=FALSE, fig.height=5, fig.width=10, results='hide'}
res <- calcMetricPerFov("Lcross", pp_ls, by = zstack_list, 
...
EXERCISE: try to compare the colocalisation of `OD Mature` and `Ependymal`cells. Again, the documentation of `r BiocStyle::CRANpkg('spatstat.explore')` will help you to find good cross functions and `calcMetricPerFov` is a helper (wrapper) function that is useful to operate on `r BiocStyle::Biocpkg('SpatialExperiment')` objects.

```{r, eval = FALSE}
res <- calcMetricPerFov()

# EXERCISE:
# 1. Compute your "cross" function of choice for comparing `OD Mature`
#   and `Ependymal` cells.
# 2. plot the curves
```
