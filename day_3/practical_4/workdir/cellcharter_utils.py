...
from cellcharter.pl._utils import adjust_box_widths
...
def _plot_shapes(data, x, y, hue, hue_order, figsize, title: str | None = None) -> None:
...
def plot_shape_metrics(
    ...
    if cluster_id is not None:
        ...
        if cluster_key is not None:
            _plot_shapes(metrics_melted, "metric", "value", cluster_key, cluster_id, figsize, f'Spatial domains: {", ".join([str(cluster) for cluster in cluster_id])} by domain')

        if condition_key is not None:
            _plot_shapes(metrics_melted, "metric", "value", condition_key, condition_groups, figsize, f'Spatial domains: {", ".join([str(cluster) for cluster in cluster_id])} by condition')
    else:
        for metric in metrics:
            _plot_shapes(
                metrics_df,
